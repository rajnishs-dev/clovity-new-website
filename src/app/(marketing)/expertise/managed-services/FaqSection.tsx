...
import { MS_FAQ, MS_FAQ_CONTENT } from '@/constants/managed-services';

/**
 * "Before You Hand Over Admin Rights" - six disclosures.
 *
 * Native `<details>`/`<summary>` rather than the shared `<Accordion>`, matching the ITSM
 * and Careers FAQs: several answers may be open at once, `<details>` needs no JS so the
 * section stays a Server Component, and the plus-to-cross rotation does not map onto
 * `Accordion`'s chevron.
 *
 * The marker is suppressed twice on purpose - `list-none` for browsers that honour
 * `::marker`, the webkit selector for Safari, which does not. Dropping either leaves a
 * stray triangle in one engine.
 *
 * FAQPage JSON-LD is emitted from the same array that renders, so the structured data
 * cannot drift from the visible answers. That matters more than usual here: the
 * response-times answer is the page's position on what it will not promise, and a
 * stale copy of it in the search result would be worse than none.
 */
    ...
    <section className="border-t border-line-faint bg-[#eaf8ff] py-12 sm:py-16">
      ...
      <div className="mx-auto w-full max-w-[900px] px-6">
        <SectionHeader
          heading={
            <>
              {MS_FAQ_CONTENT.headingLead}
              <GradientText>{MS_FAQ_CONTENT.headingHighlight}</GradientText>
            </>
          }
